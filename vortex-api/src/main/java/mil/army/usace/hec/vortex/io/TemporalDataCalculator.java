...
class TemporalDataCalculator {
    ...
    static float[] calculateWeightedAccumulation(List<VortexGrid> grids, ZonedDateTime startTime, ZonedDateTime endTime) {
    ...
    static float[] calculateWeightedAverage(List<VortexGrid> grids, ZonedDateTime startTime, ZonedDateTime endTime) {
    ...
    static float[] calculatePointInstant(List<VortexGrid> vortexGrids, ZonedDateTime time) {
    ...
    static float[] calculatePeriodInstant(List<VortexGrid> grids, ZonedDateTime startTime, ZonedDateTime endTime) {
        if (hasNoTargetData(grids, startTime, endTime)) return undefinedGridData();
    ...
    static float[][] getMinMaxForGrids(List<VortexGrid> grids) {
    ...
    /* Helpers */
    static VortexGrid buildGrid(VortexGrid baseGrid, ZonedDateTime startTime, ZonedDateTime endTime, float[] data) {
    ...
    private static float[] calculateWeightedData(List<VortexGrid> grids, double[] weights) {
        ...
        for (int gridIndex = 0; gridIndex < numGrids; gridIndex++) {
            ...
            for (int dataIndex = 0; dataIndex < numData; dataIndex++) {
                ...
                boolean isNaN = result[dataIndex] == noDataValue || data == noDataValue;
    ...
    private static double[] calculateAccumulationWeights(List<VortexGrid> grids, ZonedDateTime start, ZonedDateTime end) {
        VortexDataInterval queryRecord = VortexDataInterval.of(start, end);
        return grids.stream()
    ...
    private static double[] calculateAverageWeights(List<VortexGrid> grids, ZonedDateTime start, ZonedDateTime end) {
        ...
        List<VortexDataInterval> timeRecords = grids.stream().map(VortexDataInterval::of).toList();
        long totalDuration = end.toEpochSecond() - start.toEpochSecond();

        VortexDataInterval queryRecord = VortexDataInterval.of(start, end);
        for (int i = 0; i < timeRecords.size(); i++) {
            VortexDataInterval timeRecord = timeRecords.get(i);
    ...
    private static double[] calculateAverageOfTwoGrids(VortexGrid firstGrid, VortexGrid secondGrid, ZonedDateTime start, ZonedDateTime end) {
        VortexDataInterval timeRecord = VortexDataInterval.of(firstGrid.startTime(), secondGrid.startTime());
        VortexDataInterval queryRecord = VortexDataInterval.of(start, end);
