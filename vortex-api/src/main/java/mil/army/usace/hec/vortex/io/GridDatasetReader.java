...
import static mil.army.usace.hec.vortex.io.GridDatasetReader.SpecialFileType.UNDEFINED;
import static mil.army.usace.hec.vortex.io.GridDatasetReader.SpecialFileType.values;
...
class GridDatasetReader extends NetcdfDataReader {
    ...
    enum SpecialFileType {
        ...
        boolean matches(String fileName, String variableName) {
    ...
    @Override
    public VortexData getDto(int timeIndex) throws DataReadException {
        Objects.checkIndex(timeIndex, getDtoCount());
    ...
    /* Grid Data Helpers */
    private List<VortexData> getMultiDimensionGrids(Dimension timeDim, Dimension endDim, Dimension rtDim) throws DataReadException {
        ...
        for (int rtIndex = 0; rtIndex < rtDim.getLength(); rtIndex++) {
            for (int endIndex = 0; endIndex < endDim.getLength(); endIndex++) {
                for (int timeIndex = 0; timeIndex < timeDim.getLength(); timeIndex++) {
                    gridList.add(createGridFromDataSlice(rtIndex, endIndex, timeIndex));
                }
            }
        }
    ...
    private List<VortexData> getTemporalGrids() throws DataReadException {
        List<VortexData> gridList = new ArrayList<>();
        for (int i = 0; i < timeBounds.size(); i++) {
            gridList.add(getDto(i));
        }
        return gridList;
    }

    private List<VortexData> getStaticGrid() throws DataReadException {
        return List.of(createGridFromDataSlice(0, 0, 0));
    }

    /**
     * Reads a single grid slice from the underlying NetCDF dataset.
     *
     ...
     * @return a VortexGrid object representing the specified data slice.
     * @throws DataReadException if the underlying read fails.
     */
    private VortexGrid createGridFromDataSlice(int rtIndex, int endIndex, int timeIndex) throws DataReadException {
        try {
            ...
            VortexDataInterval timeRecord = (timeIndex < timeBounds.size()) ? timeBounds.get(timeIndex) : VortexDataInterval.UNDEFINED;
            return buildGrid(data, timeRecord);
        } catch (IOException e) {
            String slicePath = "rt=" + rtIndex + ",ens=" + endIndex + ",t=" + timeIndex;
            throw DataReadException.ioError(path, slicePath,
                    "Failed to read NetCDF grid slice [" + path + " : " + slicePath + "]: " + e.getMessage(),
                    e);
        }
    }
    ...
    /* Time Record Helpers */
    private List<VortexDataInterval> getTimeBounds(GridCoordSystem gcs) {
        if (gcs.hasTimeAxis1D()) {
            return getTimeRecordsWithTimeAxis1D(gcs.getTimeAxis1D());
        } else if (gcs.hasTimeAxis()) {
            return this.getDataIntervals();
        } else {
            VortexDataInterval timeRecord = getTimeRecordFromAttributes();
            return timeRecord != null ? List.of(timeRecord) : Collections.emptyList();
        }
    }

    private VortexDataInterval getTimeRecordFromAttributes() {
        // No time axes found. Try reading times from attributes.
        VortexDataInterval startStopDateRecord = getTimeRecordFromStartStopDate();
        return startStopDateRecord != null ? startStopDateRecord : getTimeRecordFromNominalProductTime();
    }

    private VortexDataInterval getTimeRecordFromStartStopDate() {
        ...
        return VortexDataInterval.of(startDate, endDate);
    }

    private VortexDataInterval getTimeRecordFromNominalProductTime() {
        ...
        return VortexDataInterval.of(time, time);
    }
    ...
    private List<VortexDataInterval> getTimeRecordsWithTimeAxis1D(CoordinateAxis1DTime tAxis) {
        ...
        for (int i = 0; i < (int) tAxis.getSize(); i++) {
            VortexDataInterval timeRecord = adjustTimeForSpecialFile(tAxis, i);
            list.add(timeRecord);
        }
    ...
    private VortexDataInterval adjustTimeForSpecialFile(CoordinateAxis1DTime tAxis, int time) {
