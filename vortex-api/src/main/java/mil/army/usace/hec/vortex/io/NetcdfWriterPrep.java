...
final class NetcdfWriterPrep {
    ...
    // Map
    private static final Map<String, IndexMap<VortexDataInterval>> recordIndexMap = new HashMap<>();

    private NetcdfWriterPrep() {
        // Utility Class
    }

    public static void initializeForAppend(String destination, VortexGridCollection gridCollection, Stream<VortexDataInterval> sortedRecordStream) {
        ...
        try (NetcdfFormatWriter writer = builder.build()) {
            ...
            List<VortexDataInterval> sortedRecordList = sortedRecordStream.toList();
    ...
    public static int getTimeRecordIndex(String destination, VortexDataInterval timeRecord) {
    ...
    private static void writeTimeDimension(NetcdfFormatWriter writer, List<VortexDataInterval> timeRecords) throws InvalidRangeException, IOException {
        int numData = timeRecords.size();

        long[][] timeBoundsData = new long[numData][2];
        long[] midTimeData = new long[numData];

        ChronoUnit minimumUnit = NetcdfTimeUtils.getMinimumDeltaTimeUnit(timeRecords);
        for (int i = 0; i < numData; i++) {
            VortexDataInterval timeRecord = timeRecords.get(i);
            long startTime = NetcdfTimeUtils.getNumIntervalsFromBaseTime(timeRecord.startTime(), minimumUnit);
            long endTime = NetcdfTimeUtils.getNumIntervalsFromBaseTime(timeRecord.endTime(), minimumUnit);
            long midTime = (startTime + endTime) / 2;

            timeBoundsData[i][0] = startTime;
            timeBoundsData[i][1] = endTime;
