...
// The vendored gdal dylibs reference their siblings as @rpath/libfoo.dylib but
// carry no LC_RPATH of their own (confirmed with a Mach-O parse of the actual
// artifacts), so without this they'd fail to load: jpackage launchers have no
// way to set DYLD_LIBRARY_PATH the way the old run-vortex.sh script did.
// Requires Xcode Command Line Tools (install_name_tool) on the build machine.
val jpackageFixMacRpaths = tasks.register("jpackageFixMacRpaths") {
    group = "distribution"
    description = "Adds an @loader_path rpath to the staged macOS gdal dylibs."
    ...
    doLast {
        fileTree(jpackageInputDirMacOS.get().asFile.resolve("gdal")) {
            ...
            exec {
                commandLine("install_name_tool", "-add_rpath", "@loader_path", dylib.absolutePath)
